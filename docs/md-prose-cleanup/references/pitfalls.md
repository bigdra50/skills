...
同じルールパッケージを severity 違いで 2 回読ませると、2 つ目はコメントで名前指定できなくなる。
`prh` と `textlint-rule-prh` を別キーで宣言すると severity は分けられるが、
ルール名を書いた `textlint-disable` は、どちらの名前でも 2 つ目に届かない。
実測では名前なしの `textlint-disable` だけが効いた。
2 つ目のインスタンスを部分的に外したい箇所があるなら、この構成は選ばない。
