...
> 今日のClaude Codeセッションログからマークダウン形式の作業記録を自動生成する
...
```bash
apm install bigdra50/skills/workflow/cc-worklog
```
...
dependencies:
  apm:
    - bigdra50/skills/workflow/cc-worklog
```
...
```bash
npx skills add bigdra50/skills --skill worklog
```

Or copy the folder in directly:

```bash
npx degit bigdra50/skills/workflow/cc-worklog ~/.claude/skills/worklog
```
